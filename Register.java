...
public class Register extends JFrame{
	...
	static String fileNm = "data\\log_in.txt"; //회원가입시 ID와 비밀번호가 저장될 text
	public Register(){
		setTitle("회원가입");
		grid.setVgap(5);
		field.setLayout(grid);
		text_id = new JTextField("");
		JLabel RIdLabel = new JLabel("ID : "); //코드 수정
		RIdLabel.setFont(new Font("A", Font.PLAIN, 30)); //코드 수정
		field.add(RIdLabel); //코드 수정
		field.add(text_id);
		text_password = new JPasswordField(""); 
		JLabel RPwdLabel = new JLabel("비밀번호 : "); //코드 수정
		RPwdLabel.setFont(new Font("A", Font.PLAIN, 30)); //코드 수정
		field.add(RPwdLabel); //코드 수정
